---
name: hermes-agent
description: Configure, extend, or contribute to Hermes Agent — including features,
  CLI reference, pets/mascots, and all tooling.
version: 2.3.0
...
When the user wants max-autonomy Hermes (fewer confirmations, passwordless sudo, broader toolset), `hermes-bootstrap` handles the full hardening path: 46 `hermes config set` autonomy settings, sudoers.d NOPASSWD, and host-introspection report. Trigger: "拉满" / "最大权限" / "少问我确认". Key scripts: `references/hermes-bootstrap-host-introspect.sh` (single-pass CPU/OS/deps/port probe).

### Hermes Oracle Mode (Multi-Agent Orchestration)

When the user explicitly says "主脑模式" / "Oracle Mode" / "多 Agent 调度", load the `hermes-oracle-mode` skill. Two variants coexist:

- `autonomous-ai-agents/hermes-oracle-mode/` — Chinese-language version with detailed 7-step workflow, blockage-first rules, and delivery templates.
- `devops/hermes-oracle-mode/` — English-version with the same class-level workflow, SOUL.md placement rule, and verification checklist.

Both cover: task parse → plan → subagent dispatch → blockage-first repair → independent result verification → single validated report delivery. Keep `SOUL.md` thin (trigger rule only); the full workflow lives in the skill. Quality verification reference: `hermes-oracle-mode/references/hermes-verification-workflow.md` — hardware/service/MCP/ACP verification templates with actual command probes.

### External Coding Agents (Claude Code + Codex)

- **`claude-code`** — Anthropic's autonomous coding agent CLI. Reference in `references/codex.md` for OpenAI Codex CLI integration (sandbox caveats, worktree patterns, PR reviews).
- **Codex key patterns**: always `pty=true`; must run in git repo; use `--sandbox danger-full-access` when gateway breaks bubblewrap; `exec` for one-shots, `--full-auto` for sandboxed multi-step.
...
- **Pets / Mascots** (`hermes pets`) — animated desktop companion characters
  from the [petdex](https://petdex.dev) gallery that react to agent activity
  (idle, running a tool, reviewing, error, done). Browse with `hermes pets
  list`, install with `hermes pets install <slug> --select`, preview with
  `hermes pets show`, and disable with `hermes pets off`. Configured under
  `display.pet` in `config.yaml` (`enabled`, `slug`, `render_mode`,
  `scale`). Pets install to `<HERMES_HOME>/pets/<slug>/`. Requires a
  graphics-capable terminal (kitty, Ghostty, WezTerm, iTerm2, or sixel); a
  Unicode fallback works otherwise. Run `hermes pets doctor` to diagnose.
