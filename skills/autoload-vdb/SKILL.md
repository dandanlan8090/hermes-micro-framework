---
name: autoload-vdb
description: "Hermes 启动时自动加载 vdb 技能语义匹配（Chroma + SiliconFlow BGE-M3 云端稠密 prose 模板 + 本地 sparse lexical，leading word 2x boost → final=0.6dense+0.4sparse）。触发：Hermes 初始化/启动/新 session/重新加载技能。禁用：离线环境/无 API Key/不需要语义匹配。"
version: 6.0.0
author: Hermes Agent
license: MIT
...
query
  ...
  │     输入: 仅 trigger_tags（完全隔离英文 description）
  │     leading word 命中 2x boost（池 21 词：fog-of-war/verify-first/root-cause 等）
  │     算法: tokenize(中文按字+英文按词) → log(1+tf) → 余弦归一化
  │     与 BGE-M3 compute_lexical_matching_score 算法对齐
  │
  └── final = 0.6 × dense + 0.4 × sparse → disable 过滤 → top-5
```
...
| 稀疏 (sparse) | 本地 CPU | **仅 trigger_tags**（leading word 2x boost） | 词权重字典 → Chroma metadata |

**关键约束**：sparse 只由 trigger_tags 产生，英文 description **完全不参与** sparse 计算。这确保 "写 SKILL.md" 不会因为 research-paper-writing 的 description 含中文"写"而赢过 skill-authoring 的英文 description。
...
1. 按 NEW_SKILL_TEMPLATE.md 写 frontmatter（trigger ≥5，disable 从 DISABLE_TAG_POOL 选取）
2. 重建索引：
   ```bash
