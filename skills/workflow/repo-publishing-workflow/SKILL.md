...
1. **没有真实路径/用户名/主机名泄露**
   ```bash
   # 扫所有待提交文件
   git diff --cached --name-only | xargs grep -l "/home/[a-z]\+\|C:\\\\Users\\\\\|fnubuntu" 2>/dev/null
   # 期望输出为空
   ```
...
skill 的 `author:` 字段在公开 repo 中**必须使用通用名**，不得使用 `Hermes-fn` 等内部代号：
- ✅ `author: Hermes Agent`
- ❌ `author: Hermes-fn`
- ❌ `author: dandanlan8090`

脱敏时一并替换：本地 `Hermes-fn` → repo `Hermes Agent`。
...
| 主机名（如 `fnubuntu`） | `[HOSTNAME]` |

**扫描命令：**

```bash
# 扫描所有 md 文件
grep -rn "Hermes-fn\|hermes-fn\|/home/[a-z]\+\|C:\\\\Users\\\\\|token\|ghp_\|pat_\|sk-\|API_KEY" --include="*.md" .

# 扫描路径相关
grep -rn "dandanlan\|lan\|your_username\|fnubuntu" --include="*.md" .
...
2. **技能正文中的本地路径**：`/home/lan/.hermes` → `~/.hermes/`
...
- [ ] author 字段不含 `Hermes-fn` 等内部代号（应使用 `Hermes Agent`）
...
echo "=== 个人路径扫描 ==="
git diff --cached --name-only | xargs grep -lE "/home/[a-z]+|C:\\\\Users\\\\|fnubuntu" 2>/dev/null || echo "OK"
...
echo "=== Hermes-fn 代号扫描 ==="
git diff --cached | grep -iE "Hermes-fn" || echo "OK: no internal codename"

echo "=== author 字段 ==="
git diff --cached | grep -E "author:.*Hermes-fn" || echo "OK: author safe"
```
